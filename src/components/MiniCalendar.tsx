...
export const MiniCalendar: React.FC = () => {
  ...
  return (
    <div className="bg-white rounded-xl border border-navy-100 p-3.5 shadow-sm space-y-3 h-full flex flex-col justify-between">
      <div>
        {/* Calendar Header */}
        <div className="flex items-center justify-between pb-2 mb-2 border-b border-navy-100">
          ...
          <div className="flex items-center space-x-1">
            <button
              onClick={handleToday}
              className="px-1.5 py-0.5 text-[10px] font-bold text-navy-700 hover:text-navy-900 bg-navy-50 hover:bg-navy-100 rounded border border-navy-200 transition-colors mr-0.5"
            >
              Today
            </button>
            <button
        ...
        <div className="grid grid-cols-7 gap-0.5 text-center mb-1">
          {dayNames.map((day, idx) => (
            <div key={idx} className="text-[10px] font-bold text-navy-400 py-0.5">
              {day}
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-7 gap-0.5 text-center text-[11px]">
          ...
          {Array.from({ length: firstDayOfMonth }).map((_, idx) => {
            ...
            return (
              <div
                key={`prev-${idx}`}
                className="py-1 text-navy-300 pointer-events-none select-none"
              >
                {dayNum}
              </div>
            );
          ...
          {Array.from({ length: daysInMonth }).map((_, idx) => {
            ...
            return (
              <button
                ...
                className={`py-1 rounded-md font-bold transition-all relative flex flex-col items-center justify-center ${
                  isSelected
      ...
      <div className="pt-2 border-t border-navy-100 mt-2">
        <div className="text-[10px] font-bold text-navy-400 uppercase tracking-wider mb-1">
        ...
        {selectedEvents.length > 0 ? (
          <div className="space-y-1.5">
            {selectedEvents.map((evt, i) => (
              <div key={i} className="p-2 rounded-lg bg-navy-50 border border-navy-100">
                <div className="flex items-center justify-between mb-0.5">
                  <span className="text-[9px] font-bold px-1.5 py-0.2 rounded bg-white text-navy-800 border border-navy-200 uppercase">
                    {evt.category}
                  </span>
                  {evt.time && (
                    <span className="text-[9px] text-navy-500 font-medium flex items-center">
                      <Clock className="w-2.5 h-2.5 mr-0.5" />
                      {evt.time}
                    </span>
                  )}
                </div>
                <h5 className="text-[11px] font-bold text-navy-900 leading-snug">{evt.title}</h5>
                {evt.location && (
                  <p className="text-[9px] text-navy-500 flex items-center mt-0.5">
          ...
          <div className="p-2 text-center bg-navy-50/50 rounded-lg border border-navy-100 text-[10px] text-navy-400 font-medium">
