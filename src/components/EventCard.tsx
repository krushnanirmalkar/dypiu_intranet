...
export const EventCard: React.FC<EventCardProps> = ({ event }) => (
  <article className="flex gap-2.5 border-b border-navy-100 py-2 last:border-b-0">
    ...
    <div className="min-w-0"><h4 className="truncate text-[10px] font-extrabold text-navy-950">{event.title}</h4><p className="mt-0.5 text-[9px] font-semibold text-navy-600">{event.date}</p><p className="mt-0.5 flex items-center gap-1 text-[8px] text-navy-400"><Clock className="h-2.5 w-2.5" />{event.time}</p></div>
  </article>
);
