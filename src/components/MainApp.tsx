...
import { ArrowRight, ArrowUpRight, BookOpen, CalendarDays, CheckSquare, FileText, Headphones, MessageSquare, Newspaper, ShieldCheck, UserRound, Users } from 'lucide-react';
...
const isLegacyUiEnabled = (): boolean => false;
const DYPIU_LOGO_URL = 'https://www.dypiu.ac.in/assets/images/home/logo.webp';
...
const RebuildShell: React.FC<RebuildShellProps> = ({
  user,
  currentRole,
  currentNav,
  onNavigate,
  ...
  const firstName = userName.trim().split(/\s+/)[0]?.toUpperCase() || 'THERE';
  const isProfileActive = currentNav === 'profile';

  return (
    <div className="min-h-screen bg-white font-sans">
      <aside className="fixed inset-y-0 left-0 flex w-[clamp(230px,21vw,330px)] flex-col bg-navy-950 text-white">
        <div className="min-h-[25vh] border-b border-white/10">
          <div className="flex min-h-[112px] items-center bg-white py-5 pl-10 pr-7 lg:pl-12 lg:pr-8">
            <img
              src={DYPIU_LOGO_URL}
          ...
          <div className="grid h-1 grid-cols-4" aria-hidden="true">
            <span className="bg-[#18b78c]" />
            <span className="bg-[#1f86c7]" />
            <span className="bg-[#f5b21b]" />
            <span className="bg-[#ef6a24]" />
          </div>

          <div className="px-7 py-7 lg:px-8 lg:py-8">
            <p className="text-[clamp(1.45rem,1.8vw,1.9rem)] font-extrabold leading-none tracking-[-0.035em] text-white">
              {greeting},
            </p>
            <h1 className="mt-2 text-[clamp(2rem,2.8vw,2.8rem)] font-black leading-none tracking-[-0.045em] text-[#f4b223] drop-shadow-sm" title={userName}>
              {firstName}!
            </h1>
            <p className="mt-5 text-[clamp(0.85rem,1.05vw,1rem)] font-medium leading-relaxed text-white/75">
              Let&apos;s make today productive.
            </p>
            <div className="mt-5 h-[3px] w-16 rounded-full bg-[#f4b223]" aria-hidden="true" />
          </div>
        </div>

        <nav className="mt-auto px-7 py-6 lg:px-8" aria-label="Account navigation">
          <button
            type="button"
            onClick={() => onNavigate(isProfileActive ? 'dashboard' : 'profile')}
            className={`flex h-10 w-10 items-center justify-center rounded-full border transition ${
              isProfileActive
                ? 'border-white bg-white text-navy-950 shadow-md shadow-black/20'
                : 'border-white/15 bg-white/[0.05] text-white/70 hover:border-white/30 hover:bg-white/10 hover:text-white'
            }`}
      ...
      <main className="min-h-screen ml-[clamp(230px,21vw,330px)]" aria-label="Main content">
        {isProfileActive ? (
          <div className="p-5 sm:p-7 lg:p-8">
            <ProfilePage user={user} currentRole={currentRole} />
          </div>
        ) : (
...
export const MainApp: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#f7f9fd] font-sans text-navy-900">
      ...
      <TopNavbar
        user={currentUser}
        overlaysHero={currentNav === 'dashboard'}
      ...
      <main className={`mx-auto w-full max-w-[1500px] px-4 sm:px-5 lg:px-6 ${currentNav === 'dashboard' ? '-mt-16 pb-3 lg:pb-4' : 'py-3 lg:py-4'}`}>
            ...
            <section className="overflow-hidden rounded-[14px] border border-[#e5ebf5] bg-white shadow-[0_3px_12px_rgba(15,35,75,0.03)]">
              <div className="grid grid-cols-2 divide-x divide-y divide-navy-100 sm:grid-cols-[150px_repeat(4,minmax(0,1fr))] sm:divide-y-0">
                <div className="col-span-2 flex items-center px-4 py-3 sm:col-span-1">
                  <div><p className="text-[12px] font-bold uppercase tracking-[0.14em] text-blue-600">Start here</p><h3 className="mt-0.5 text-[12px] font-extrabold text-navy-950">Quick Access</h3></div>
                </div>
